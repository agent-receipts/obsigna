...
Publishing uses npm **OIDC trusted publishing** (configured on the npm package), not a long-lived token: `id-token: write` + setup-node's `registry-url` (for OIDC context) + `npm publish --provenance`. Unlike `release-sdk-ts.yml`, the publish step force-clears `NODE_AUTH_TOKEN` — this repo injects a runner-level token (which covers `@obsigna/sdk-ts`, so sdk-ts publishes with it) that does **not** cover `@obsigna/opencode-plugin`; a non-empty token in `.npmrc` takes precedence over OIDC, so it must be cleared for the OIDC path to run. npm then mints a short-lived credential from the GitHub Actions OIDC identity and signs provenance from it. Managing dist-tags (`npm dist-tag`) and deprecations is out of band — OIDC covers publish only, so those still need a human-held npm credential.
