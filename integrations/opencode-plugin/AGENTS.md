...
Publishing uses npm **OIDC trusted publishing** (configured on the npm package), not a long-lived token — the same setup as `release-sdk-ts.yml`: `id-token: write` + setup-node's `registry-url` + `npm publish --provenance`, no token. npm attempts OIDC first, mints a short-lived credential, and signs provenance from it. The npm Trusted Publisher must point at repository **`obsigna`** (the monorepo, not the package) with workflow filename `release-opencode-plugin.yml`; a wrong field makes OIDC silently fall back to auth that lacks access and the publish 404s. Managing dist-tags (`npm dist-tag`) and deprecations is out of band — OIDC covers publish only, so those still need a human-held npm credential.
