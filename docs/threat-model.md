...
**Mitigation: external anchor checkpoints (ADR-0008), shipped end-to-end.** The daemon emits small Ed25519-signed checkpoints — `{chain_id, sequence, receipt_hash, timestamp}` — to one or more operator-configured append-only sinks (`--checkpoint-anchor`: `file:`, `git:`, `syslog:`) every `--checkpoint-cadence` receipts, with a forced checkpoint on graceful shutdown. `receipt_hash` commits to the chain HEAD, so `obsigna verify --against-anchor` detects truncation when the anchored head sits ahead of the store head — a mismatch on `sequence` or `receipt_hash`. The checkpoint is out-of-band and additive: it never touches the receipt schema, the hash chain, or the issuer DID (ADR-0008). The available sinks are dependency-free *reference* adapters (a plain file is only as append-only as the filesystem around it); production-grade sinks (S3 object-lock, transparency log, SIEM ingest) are still landing.
...
| Key rotation, BYOK, external anchoring | [ADR-0015](adr/0015-key-rotation-byok-anchoring.md), [ADR-0008](adr/0008-response-hashing-and-chain-completeness.md) | [#307](https://github.com/agent-receipts/obsigna/issues/307) (PR [#319](https://github.com/agent-receipts/obsigna/pull/319)) | Accepted; rotation + anchor-first writes + checkpoint anchoring (truncation detection, ADR-0008) shipped with file/git/syslog reference sinks; production anchor adapters (S3 object-lock, transparency log, SIEM) & HSM/KMS (Phase C) pending |
