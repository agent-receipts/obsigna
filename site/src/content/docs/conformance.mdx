...
The counts are the real sizes of the frozen corpora today, not a coverage claim. They are seed-level and growing: 43 canonicalization/receipt-hash vectors, a 7-case MUST-reject corpus, and 2–3 pinned receipt/chain vectors per spec version. They are **not** exhaustive — they pin the shapes that mattered enough to freeze, and the corpus is extended as the specification evolves. The **did:key resolution** vectors are reference fixtures that fix the `did:key` v0.7 wire shape (ADR-0007); they are not yet wired into any SDK test suite, so their row is marked `—` for all three implementations rather than claiming verification that does not yet run.
</Aside>
