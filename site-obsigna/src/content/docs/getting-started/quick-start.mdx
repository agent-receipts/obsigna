...
```bash
obsigna-daemon --init   # one-time: signing + forensic keys, and a config with disclosure on
...
with DaemonEmitter() as e:  # uses AGENTRECEIPTS_SOCKET or the per-OS default
    e.emit(
        ...
        decision="allowed",
        input='{"path": "/etc/hosts"}',  # raw JSON; the daemon hashes it and (with the --init default) HPKE-encrypts it so it is recoverable
    )
```
...
async function main() {
  ...
  try {
    ...
    const err = await e.emit({
      ...
      decision: "allowed",
      input: JSON.stringify({ path: "/etc/hosts" }), // raw JSON; hashed by the daemon and (with the --init default) HPKE-encrypted so it is recoverable
    });
    if (err) throw err;
  } finally {
    e.close();
...
func main() {
	...
	if err := e.Emit(context.Background(), emitter.Event{
		...
		Decision: "allowed",
		Input:    json.RawMessage(`{"path":"/etc/hosts"}`), // raw JSON; hashed by the daemon and (with the --init default) HPKE-encrypted so it is recoverable
	}); err != nil {
		log.Fatal(err)
	}
}
...
Verification proves the receipt is intact, but not *what the agent did*. Because `--init`
turns parameter disclosure on, each receipt also carries the tool's raw input, encrypted
to your forensic key. Recover it with `obsigna receipt disclose`:

```bash
obsigna receipt disclose 1          # decrypt receipt 1's parameters
# path: /etc/hosts
```

`disclose` decrypts locally with the forensic private key (`~/.local/share/agent-receipts/forensic.key`,
written by `--init`); the plaintext is never written back to the store. This is the
"break the glass" path — the store keeps ciphertext, and only a holder of that key can
read it. To record hashes alone instead, set `parameter_disclosure = false` (see
[Payload disclosure](/getting-started/daemon-setup/#payload-disclosure)).
