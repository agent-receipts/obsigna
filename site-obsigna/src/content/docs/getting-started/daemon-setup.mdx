...
| Forensic private key | `$XDG_DATA_HOME/agent-receipts/forensic.key` (same fallback) |
| Forensic public key | `$XDG_DATA_HOME/agent-receipts/forensic.key.pub` (same fallback) |
| Starter config | `$XDG_DATA_HOME/agent-receipts/daemon.toml` (same fallback) |

`--init` sets up a complete install in one step. Alongside the Ed25519 signing pair it generates an X25519 **forensic key pair** ([ADR-0012](https://github.com/agent-receipts/obsigna/blob/main/docs/adr/0012-parameter-disclosure.md)) and writes a `daemon.toml` that turns **parameter disclosure on**. The result: a fresh daemon records the raw tool inputs — encrypted into each receipt and recoverable with the forensic private key — not just hashes. See [Payload disclosure](#payload-disclosure) below for what that means and how to read a recorded payload.

The public key is needed by `obsigna receipt verify` — keep it alongside the database when archiving. The **forensic private key** (`forensic.key`) is the recovery secret: whoever holds it can decrypt every recorded payload, and the daemon never reads it. For a real deployment, move `forensic.key` off the daemon host once `--init` has run — the daemon only ever needs `forensic.key.pub`.

Override the key paths with environment variables: `AGENTRECEIPTS_KEY` (signing key), `AGENTRECEIPTS_PUBLIC_KEY` (public key).
...
`--init` refuses to overwrite an existing signing or forensic key and exits non-zero, and never clobbers a `daemon.toml` you have already edited (it reports that it left your config untouched). It is a one-time bootstrap, not a rotation command — run it once per machine.
...
forensic_public_key = "/home/me/.local/share/agent-receipts/forensic.key.pub"  # --forensic-public-key
parameter_disclosure = "true"         # --parameter-disclosure; "true" (the --init default) | "high" | "fs.write,net.http" | false
...
After `--init`, parameter disclosure is **on**. Each tool call's parameters are encrypted to the forensic X25519 public key using HPKE (ADR-0012) and ride in the signed receipt's `parameters_disclosure` field; only the holder of the matching `forensic.key` can decrypt them, and the daemon itself cannot read what it wrote. The `parameters_hash` is always present too, so tamper-evidence never depends on disclosure.

This is the default because a hash alone often isn't enough to answer "what did the agent actually do?". With disclosure on you can recover the exact inputs of any recorded action:

```bash
obsigna receipt disclose 42                      # recover receipt 42's parameters
obsigna receipt disclose 42 --json               # as JSON
```

`disclose` reads the forensic **private** key (default `~/.local/share/agent-receipts/forensic.key`; override with `--key` or `AGENTRECEIPTS_FORENSIC_KEY`) and decrypts the envelope locally — the plaintext is never written back to the store.

### Choosing a disclosure policy

`parameter_disclosure` selects *which* actions disclose. `--init` writes `"true"` (every action) so a fresh install shows everything; narrow it by editing `daemon.toml` (or the `--parameter-disclosure` flag / `AGENTRECEIPTS_PARAMETER_DISCLOSURE`):

| Value | Discloses |
|-------|-----------|
| `"true"` | every action (the `--init` default) |
| `"high"` | high-risk actions only |
| `"fs.write,net.http"` | a comma-separated action-type allowlist |
| `false` | nothing — hashes only |

Any enabled policy requires a forensic public key to encrypt to; `--init` provides one. To turn disclosure **off**, set `parameter_disclosure = false`.

> **Production:** the forensic private key is the recovery secret. Move `forensic.key` off the daemon host after `--init` (the daemon only needs `forensic.key.pub`), so a host compromise cannot decrypt recorded payloads. This is also what makes crypto-shredding possible — destroy the private key and the encrypted payloads become unrecoverable while the hash-based audit trail stays verifiable.
