...
| `-against-anchor` | Path to an out-of-band signed checkpoint log (ADR-0008). When set, additionally checks the chain HEAD against the latest checkpoint and fails on tail truncation (env: `AGENTRECEIPTS_AGAINST_ANCHOR`) |
| `-json` | Emit a single machine-readable JSON object instead of human-readable text |

A successful run prints `Chain <id>: VALID (<n> receipts)` (or `receipt` when n=1). Chain verification proves signatures, hash linkage, and sequence numbers on the receipts it was given — it does **not** by itself prove none were deleted (spec §7.3.1), so without `--against-anchor` the verdict is qualified: `... — truncation not checked. Pass --against-anchor to check against an out-of-band checkpoint.` An empty chain (no receipts at all) is reported separately as `Chain <id>: NO RECEIPTS FOUND (0 receipts)` rather than `VALID`, since an empty result trivially passes every check and is not evidence the chain never existed or wasn't deleted. A tampered chain prints `Chain <id>: BROKEN at receipt <n>` with the failing cause and per-receipt status (`BAD SIGNATURE`, `BAD HASH LINK`, `BAD SEQUENCE`).

**Exit codes:** `0` chain verified (including an empty or unqualified-truncation result); `1` chain failed verification, or a `--against-anchor` check failed; `2` bad flags or an unreadable DB/key file.
...
Decrypt the `parameters_disclosure` envelope of a single stored receipt using the operator's X25519 forensic private key, and print the recovered plaintext parameters. A fresh install set up with `obsigna-daemon --init` records this envelope on every receipt, encrypted to your forensic key; the store itself never holds plaintext. `disclose` is the controlled "break the glass" path for an operator who holds the forensic private key — the headless counterpart to the dashboard's inline forensic decryption.
