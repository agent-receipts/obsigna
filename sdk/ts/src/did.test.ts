...
describe("didFromPublicKey", () => {
	it("derives the did:key identifier from a public key", () => {
		expect(didFromPublicKey(Buffer.from(PUBLIC_KEY_HEX, "hex"))).toBe(
			EXPECTED_DID,
		);
	});
...
describe("resolveDid", () => {
	...
	it("rejects an oversized encoded payload before decoding it", () => {
		// Guards the DoS fix: resolveDid must reject oversized input before
		// running the O(n^2) base58btc decode on it, not just after.
		const oversized = "z".repeat(MAX_ENCODED_LEN + 1);
		expect(() => resolveDid(`did:key:z${oversized}`)).toThrow(
			/exceeds maximum/,
		);
	});

	it("rejects an unsupported multicodec", () => {
		const wrongEd = base58btcEncode(
			new Uint8Array([0xed, 0x02, ...new Array(32).fill(0)]),
		);
		expect(() => resolveDid(`did:key:z${wrongEd}`)).toThrow(/multicodec/);

		const secp256k1 = base58btcEncode(
